...
# run this to have nlp model returned to us
def train_spacy(data, iterations): # iterations = EPOCs, sometimes youre gonna do more or less
    TRAIN_DATA = data
    nlp = spacy.blank("en")

    if "ner" not in nlp.pipe_names: # if model doesn't have a ner pipename
        #ner = nlp.create_pipe("ner") # create ner pipeline
    ...
    with nlp.disable_pipes(*other_pipes):
        ...
        for itn in range(iterations):
            ...
            random.shuffle(TRAIN_DATA) # common practice to randomize training data
            losses = dict()
            ...
            for text, annotations in TRAIN_DATA:
                ...
                nlp.update(
                    [example],
                    drop=0.2, # standard practice - prevents overfitting
                    sgd=optimizer, 
                    losses=losses
                )
                print(losses)
...
# nlp.to_disk("hp_ner_model") # save trained model
...
doc = nlp(test) # test the model

for ent in doc.ents:
    print(ent.text, ent.label_)

# these models were able to guess because the entities were in the training data
# we are going to use word vectors to improve this